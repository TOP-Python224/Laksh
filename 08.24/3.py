...
def remove_numbers(numbers: list[Real],
    ...
    if modify_list:
        # ИСПОЛЬЗОВАТЬ: когда нам не нужна переменная цикла, то вместо имени переменной мы используем знак подчёркивания
        for _ in range(n):
...
# stdin:
# ДОБАВИТЬ: вывод сгенерированного списка до и после изменения — работы функции-то не видать
